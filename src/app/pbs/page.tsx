...
import { Plus, Trophy, Clock, Route, Zap, Trash2 } from 'lucide-react'
import { getPBs, addPB, deletePB } from '@/lib/store'
import { samplePBs } from '@/lib/seed-data'
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">All your fastest times in one place</p>
        </div>
        <button
      ...
      {pbs.length === 0 ? (
        <EmptyState
          ...
          description="Add your first PB to track your fastest times."
          actionLabel="Add PB"
                ...
                <tr key={pb.id} className="group hover:bg-secondary/30 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">{pb.distance}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Clock className="h-4 w-4 text-amber-500" />
                      <span className="font-mono font-bold text-lg">{pb.time}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-sm text-muted-foreground">{pb.pace}</span>
                  </td>
