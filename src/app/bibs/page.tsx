...
import { Plus, Hash, Calendar, MapPin, Trash2 } from 'lucide-react'
import { getBibs, addBib, deleteBib } from '@/lib/store'
import { sampleBibs } from '@/lib/seed-data'
...
export default function BibsPage() {
  ...
  useEffect(() => {
    const b = getBibs()
    setBibs(b.length > 0 ? b : sampleBibs)
    setIsLoaded(true)
  }, [])
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      {bibs.length === 0 ? (
        <EmptyState
          title="No bib numbers yet"
          description="Add your first race bib number."
          actionLabel="Add Bib"
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {bibs.map((bib, i) => (
            <div
              key={bib.id}
              className="group relative animate-fade-up rounded-xl border border-border bg-card p-6 transition-all hover:shadow-lg hover:-translate-y-0.5"
            ...
            >
              ...
              <div className="mb-4 flex items-center justify-center">
                <div className="flex h-28 w-36 items-center justify-center rounded-xl bg-gradient-to-br from-running-500/20 to-running-600/10 border-2 border-dashed border-running-500/30">
                  <div className="text-center">
                    <Hash className="mx-auto h-5 w-5 text-running-500 mb-1" />
                    <span className="block font-mono text-2xl font-bold tracking-widest text-running-400">
                      {bib.number}
                    </span>
                  </div>
                </div>
              ...
              {bib.notes && (
                <p className="mt-3 text-center text-xs italic text-muted-foreground border-t border-border/50 pt-3">
                  &ldquo;{bib.notes}&rdquo;
                </p>
              )}
            </div>
          ))}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-running-500 font-mono"
              placeholder="A1250"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-running-500"
              rows={3}
              placeholder="Any memories from this race?"
            />
          </div>
